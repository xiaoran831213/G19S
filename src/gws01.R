...
cfg.phe<-'DBP_1';
cfg.cov<-c('AGE_1', 'SMOKE_1');
phe<-read.table(file= "dat/phe.csv", header=TRUE, row.names=1, sep=",", stringsAsFactors=FALSE)[cfg.phe];
phe<-miss.rmv(phe, is.na)
cov<-read.table(file= "dat/phe.csv", header=TRUE, row.names=1, sep=",", stringsAsFactors=FALSE)[cfg.cov];
cov<-miss.rmv(cov, is.na)

idv.gno<-read.table(file="dat/gno/idv", header=FALSE, stringsAsFactors=FALSE)[,1]; # individual index in genotype files
idv.exp<-read.table(file="dat/exp/idv", header=FALSE, stringsAsFactors=FALSE)[,1]; # individual index in expression files.
...
rng<-read.table(file="dat/rng.txt", header=TRUE, stringsAsFactors=FALSE);
...
for (i in 1:nrow(rng))
{
	...
	gno_i<-miss.avg(gno_i, is.na); # imputate missing genotype values
	...
	capture<-tryCatch(
	{
		...
		nwu<-NHWU(Tn=P[,1], geno=G_1, X=C, gsim = 'add', appx = 'davis');
		
		K_1<-weight.gaussian(E); # get K weight matrix
		hwu<-HWU(Tn=P[,1], K=K_1, geno=G_1, X=C, gsim = 'add', appx = 'davis');
		
		k_e<-sum(K_1)/(nrow(K_1)*ncol(K_1));
		K_2<-K_1-k_e;
		pwu<-HWU(Tn=P[,1], K=K_2, geno=G_1, X=C, gsim = 'add', appx = 'davis');
	},
	warning=function(war) {return (war);},
	error=function(err)   {return (err);},
	finally =	{});
	...
	# record U core output
	{
		...
		#write.table(x=signif(hwu$coef,4),file=paste("coef_hwu/",seq_i,sep=""),quote=FALSE,sep='\t',row.names=FALSE,col.names=FALSE);
